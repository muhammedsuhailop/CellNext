  ...
  try {
    ...
    if (
      ...
      return res
        ...
        .json({ error: ADMIN_MESSAGES.ERROR.FIELDS_MISSING });
    }
    ...
    if (new Date(startOn) >= new Date(expireOn)) {
      return res.status(HttpStatusCode.BAD_REQUEST).json({
        error: "Start date must be before the expiration date.",
      });
    }
    ...
    if (maxDiscount && maxDiscount < 0) {
      return res
        .status(HttpStatusCode.BAD_REQUEST)
        .json({ error: "Maximum discount must be a positive value." });
    }
    ...
    res
      ...
      .json({ error: ADMIN_MESSAGES.ERROR.CREATE_COUPON });
  }
};
...
const deleteCoupon = async (req, res) => {
  try {
    ...
    if (!couponId)
      return res
        .status(HttpStatusCode.BAD_REQUEST)
        .json({ error: ADMIN_MESSAGES.ERROR.BAD_REQUEST });
    ...
    res
      ...
      .json({ message: ADMIN_MESSAGES.SUCCESS.COUPON_DELETED });
  } catch (error) {
