...
const loadSalesReport = async (req, res) => {
  try {
    ...
    const { paginatedResults, totalSalesCount } = await fetchSalesData({
      filterType,
      startDate,
      endDate,
      skip,
  ...
  try {
    ...
    if (paginatedResults.length === 0) {
      return res
        ...
        .send("No sales data found for the selected filter.");
    }
    ...
    const formattedStartDate =
      displayStartDate && !isNaN(displayStartDate)
        ? displayStartDate.toLocaleDateString()
        : "N/A";
    const formattedEndDate =
      displayEndDate && !isNaN(displayEndDate)
    ...
    doc
      .fontSize(10)
      .text(reportDates, doc.page.margins.left + pageWidth / 2, y, {
        width: pageWidth / 2,
        align: "right",
      })
      .moveDown(1);
      ...
      row.forEach((text, index) => {
        ...
        doc
          ...
          .text(text, x + padding, y + padding, {
            width: columnWidths[index] - 2 * padding,
            align: "left",
          });
    ...
    res
      .status(HttpStatusCode.INTERNAL_SERVER_ERROR)
      .send(ADMIN_MESSAGES.ERROR.GENERATING_PDF);
  }
};
...
const downloadExcel = async (req, res) => {
  try {
    ...
    if (filterType && filterType !== "custom") {
      const computedDates = getDateRange(filterType);
      dateFilter.createdOn = {
        $gte: computedDates.startDate,
        $lte: computedDates.endDate,
      };
    } else if (startDate && endDate) {
      dateFilter.createdOn = {
        $gte: new Date(startDate),
    ...
    if (paginatedResults.length === 0) {
      return res
        .status(HttpStatusCode.NOT_FOUND)
        .send(ADMIN_MESSAGES.ERROR.NO_SALES_DATA_FOUND);
    }
    ...
    res
      .status(HttpStatusCode.INTERNAL_SERVER_ERROR)
      .send(ADMIN_MESSAGES.ERROR.GENERATING_EXCEL);
  }
};
