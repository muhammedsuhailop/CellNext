...
const addProduct = async (req, res) => {
  try {
    ...
    const croppedImageDir = path.join(
      __dirname,
      "../../public/uploads/prod-imgs",
    );
  ...
  try {
    ...
    if (existingVariant) {
      return res.status(HttpStatusCode.BAD_REQUEST).json({
        success: false,
        message: "A variant with the same color and storage already exists.",
      });
    }
    ...
    return res
      ...
      .json({
        success: true,
        message: ADMIN_MESSAGES.SUCCESS.NEW_VARIANT_CREATED,
        variant: newVariant,
      });
  } catch (error) {
...
const addProductOffer = async (req, res) => {
  try {
    ...
    if (!findProduct.variants || findProduct.variants.length === 0) {
      return res.json({
        status: false,
        message: "No variants found for the product",
      });
    }
    ...
    if (percentage <= 0 || percentage > 100) {
      return res.json({
        status: false,
        message: "Invalid product offer percentage",
      });
    }
...
const unblockProduct = async (req, res) => {
  try {
    ...
    res.json({ message: ADMIN_MESSAGES.SUCCESS.PRODUCT_UNBLOCKED });
  } catch (error) {
...
const editVariant = async (req, res) => {
  try {
    ...
    const newImages = req.files?.variantImages
    ...
    return res
      ...
      .json({
        success: true,
        message: ADMIN_MESSAGES.SUCCESS.PRODUCT_VARIANT_UPDATED,
        variant,
      });
  } catch (error) {
      ...
      cloudinary.uploader.destroy(
        ...
        async (error, result) => {
          if (error) {
            ...
            return res
              .status(HttpStatusCode.INTERNAL_SERVER_ERROR)
              .json({ error: ADMIN_MESSAGES.ERROR.FAILED_REMOVE_IMAGE });
          }
          ...
          return res
            .status(HttpStatusCode.OK)
            .json({
              success: ADMIN_MESSAGES.SUCCESS.PRODUCT_VARIANT_IMG_REMOVED,
            });
        },
      );
    } else {
...
const updateProductVariant = async (req, res) => {
  try {
    const { productId, variantIndex, salePrice, regularPrice, stock } =
      req.body;
    ...
    if (
      ...
      return res.status(HttpStatusCode.NOT_FOUND).json({
        status: false,
        message: "Invalid product or variant index.",
      });
    }
    ...
    res
      .status(HttpStatusCode.OK)
      .json({ status: true, message: ADMIN_MESSAGES.SUCCESS.PRODUCT_UPDATED });
  } catch (error) {
    console.error("Error updating product:", error);
