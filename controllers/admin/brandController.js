  ...
  try {
    ...
    if (!name || !/^[a-zA-Z][a-zA-Z0-9 _-]*[a-zA-Z0-9]$/.test(name)) {
      return res.status(HttpStatusCode.BAD_REQUEST).json({
        error:
          "Brand name should only contain alphabets, spaces, and (-_) and start with an alphabet.",
      });
    }
    ...
    if (brandExists) {
      return res
        .status(HttpStatusCode.BAD_REQUEST)
        .json({ error: "Brand already exists." });
    }
    ...
    res
      ...
      .json({ error: ADMIN_MESSAGES.ERROR.ADDING_BRAND });
  }
};
...
const blockBrand = async (req, res) => {
  try {
    ...
    res
      ...
      .json({ error: ADMIN_MESSAGES.ERROR.BLOCKING_BRAND });
  }
};
...
const unblockBrand = async (req, res) => {
  try {
    ...
    res
      .status(HttpStatusCode.OK)
      .json({ error: ADMIN_MESSAGES.ERROR.UNBLOCKING_BRAND });
  }
};
