...
const loadDashboard = async (req, res) => {
  try {
    ...
    const orderStatusPipeline = [
      ...
      { $unwind: "$orderItems" },
      {
        $group: {
          ...
          delivered: {
            $sum: {
              $cond: [{ $eq: ["$orderItems.itemStatus", "Delivered"] }, 1, 0],
            },
          },
          cancelled: {
            $sum: {
              $cond: [{ $eq: ["$orderItems.itemStatus", "Cancelled"] }, 1, 0],
            },
          },
          returned: {
            $sum: {
              $cond: [{ $eq: ["$orderItems.itemStatus", "Returned"] }, 1, 0],
            },
          },
          cancelRequest: {
            $sum: {
              $cond: [
                { $eq: ["$orderItems.itemStatus", "Cancel Request"] },
                1,
                0,
              ],
            },
          ...
          returnRequest: {
            $sum: {
              $cond: [
                { $eq: ["$orderItems.itemStatus", "Return Request"] },
                1,
                0,
              ],
            },
    ...
    const expectedPeriods = generatePeriods(
      filter,
      revenueStartDate,
      revenueEndDate,
    );
    ...
    const totalRevenue = fullRevenueData.reduce(
    ...
    res
      ...
      .send(ADMIN_MESSAGES.ERROR.INTERNAL_SERVER_ERROR);
  }
};
