...
async function checkCouponValidity(items, categories, products, minAmount) {
  ...
  for (const item of items) {
    ...
    const isProductMatch =
      products.includes("all") || products.includes(product._id.toString());
...
const updateItemStatus = async (req, res) => {
  try {
    ...
    if (newStatus === "Cancelled" || newStatus === "Returned") {
      ...
      if (
        ...
        const remainingItems = order.orderItems.filter(
          (otherItem) =>
            otherItem._id.toString() !== item._id.toString() &&
        ...
        if (order.couponApplied) {
          ...
          if (coupon) {
            ...
            couponValid = await checkCouponValidity(
              remainingItems,
              applicableCategories,
              applicableProducts,
              minimumOrderAmount,
        ...
        order.finalAmount =
          order.subTotal + order.deliveryCharge - order.couponDiscount;
      ...
      if (
        ...
        if (userWallet) {
          ...
          if (order.couponApplied) {
            const coupon = await Coupon.findById(order.coupon);
            if (coupon) {
              const applicableCategories = coupon.applicableCategories.map(
                (id) => id.toString(),
              );
              ...
              if (!isCouponStillValid) {
                ...
                const discountPerItem =
                  order.couponDiscount / (otherItemsCount + 1);
    ...
    const allItemsSameStatus = order.orderItems.every(
      (i) => i.itemStatus === newStatus,
    );
    if (allItemsSameStatus) {
      order.status = newStatus;
    } else {
      const hasCancelRequests = order.orderItems.some(
        (i) => i.itemStatus === "Cancel Request",
      );
      const hasCancellations = order.orderItems.some(
    ...
    if (
      ...
      if (order.payment.method === "cod")
        order.payment.status =
          order.payment.method === "cod" ? "Cancelled" : "Refunded";
      order.deliveryCharge =
        order.payment.method === "cod" ? 0 : order.deliveryCharge;
    } else if (newStatus === "Returned" && allItemsSameStatus) {
      order.payment.status = "Refunded";
    }
    ...
    order.finalAmount = Math.max(
      0,
      remainingTotal - order.couponDiscount + order.deliveryCharge,
    );
